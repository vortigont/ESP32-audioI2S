...
void Audio::info(event_t e, const char* fmt, Args&&... args) {
    ...
    if(!_literal_callback && !_cb_types.test(e)) return;
    ...
    return _literal_callback(e, result.c_get());
    /*
    msg_t i;
    i.msg = result.c_get();
    i.e = e;
...
//    m_f_running = false;       // already done in stopSong
    ...
    m_streamTitle.assign(__empty_string);
    m_resumeFilePos = -1;
...
//                                                     |<-----------------------------path------------------------------------>......
    ...
    if(!hwoe.valid()) hwoe.assign(__empty_string);
    if(!extension.valid()) extension.assign(__empty_string);
    if(!path.valid()) path.assign(__empty_string);
    ...
    if(res) {
        ...
        m_f_running = false;
        // run callbacks with empty string to reset any titles/descriptions
        info(evt_name,  __empty_string);
        info(evt_streamtitle, __empty_string);
        info(evt_icydescription, __empty_string);
        info(evt_icyurl, __empty_string);
    }
    xSemaphoreGiveRecursive(mutex_playAudioData);
    return res;
}
...
//                                                     |<-----------------------------path------------------------------------------->
    ...
    if(!m_currentHost.valid()) m_currentHost.assign(__empty_string);
...
bool Audio::connecttoFS(fs::FS& fs, const char* path, int32_t fileStartTime) {
    ...
    if(c_path.ends_with_icase(".opus")) {codec = CODEC_OGG; m_f_ogg = true;}
...
void Audio::processLocalFile() {
    ...
    if(!m_f_stream) {
        if(m_codec == CODEC_OGG) { // AUDIO_LOG_ERROR("determine correct codec here");
            ...
            if     (codec == CODEC_FLAC)   {initializeDecoder(codec); m_codec = CODEC_FLAC;}
...
uint64_t Audio::getLastGranulePosition(){
    ...
    if(!buff) {AUDIO_LOG_ERROR("oom"); return 0;}
...
void Audio::enableCallbackType(event_t e, bool state){
    if (e >= _cb_types.size()) return;
    switch (e){
        case evt_none:
            if (state)
                _cb_types.reset();
